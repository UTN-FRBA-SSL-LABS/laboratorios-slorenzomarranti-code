...
> R:Inicializa el Carrito en 0, así no hay errores al empezar a contar en el carrito.
...
> R: El bug está en la línea 24 y consta de que no esta multiplicando el precio por la cantidad.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Multiplicar c->items[i] de precio y de cantidad
...
> R: El test falló y devolvió segmentation fault. (resultado 1)
...
> R: Las líneas 29 y 30, porque no están en ningun test.
...
> R: La diferencia es que el test unitario busca conflictos entre los productos por separado, y el test de integracion busca conflictos a la hora de contabilizar los productos o aplicar funciones en totalidades.
...
> R: Porque C es el único de los 3 que NO verifica los límites de los arrays, sigue escribiendo en otro apartado de memoria.
...
> R: Que programar test nos ayuda a tener una herramienta para que no vuelva a suceder en el resto del código, y nos ayude globalmente. También nos ayuda a comprender mejor el proceso de compilación.
...
> R: Porque corrobora que el precio por unidad del producto sea correcto. El problema es que el ejemplo usó Leche, que era un producto conflictivo que luego arreglamos en el test siguiente. El test siguiente funcionó porque tuvo en cuenta la cantidad.
